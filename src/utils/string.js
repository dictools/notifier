...
const string = {
  ...
  getTask: (message = "", project = {}) => {
    ...
    if (tasks && project.boardUrl) {
      return tasks.map(task => string.replace(GLOBALS.CORE.TASK, { url: project.boardUrl, task: task })).join(tasks.length > 1 && " / ");
    }
  ...
  getAttachmentFormatted: (line, type, commit, config) => {
    ...
    const description = string.replace(config.messages[type].description, { key: line.key, type });

    const body = string.replace(config.messages[type].body, {
      value: line.value,
      oldValue: line.oldValue,
    });

    const footer = string.replace(config.messages[type]?.footer || (task ? GLOBALS.MESSAGES.FOOTER_WITH_TASK : GLOBALS.MESSAGES.FOOTER), {
