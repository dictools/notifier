...
const git = {
  ...
  diff: (source) => git.run(`git diff HEAD@{1} ${source}`).then(resp => resp).catch(error => error),
  getCommitDetails: async () => {
    ...
    const commit = await git.run(`git reflog show -1 --pretty=format:'${JSON.stringify(data)}'`).then(resp => resp ? normalizeStringToJSON(normalize(resp)) : {});
